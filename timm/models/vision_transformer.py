...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # (QKV, B, Heads, N, head_dim)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C) # (B, N, Heads*head_dim)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
